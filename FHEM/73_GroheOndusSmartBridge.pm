...
use GPUtils qw(GP_Import);    # wird f¸r den Import der FHEM Funktionen aus der fhem.pl benˆtigt
...
sub WebFormResponseProcessing($$) 
{
	...
	elsif ( defined($hash->{helper}{ondusaddress}) ) 
	{
		  ...
		  and defined( $decode_json->{refresh_token} ))
		{
			...
			my $jsondata =  
	    	{
          		'refresh_token' => $hash->{helper}{token}
        	};
	...
	elsif ( $data =~ m/action=\"([^\"]*)\"/ ) 
	{
		...
		my $payload = 
			"username=" . urlEncode( AttrVal( $name, 'groheOndusAccountEmail', 'none' ) ) .
			"&password=" . urlEncode( ReadPassword($hash) );
